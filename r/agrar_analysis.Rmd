...
```{r, engine = 'bash', eval = TRUE}
paste -s -d" " ../input/names/firm_keywords.txt | fold -w 90 | head -n8
```
...
stat_controls <- tableby.control(
  test = FALSE,
  total = FALSE,
  numeric.stats = c("Sum", "N", "mean", "sd", "q1q3", "median", "min", "max"),
...
The total amount of agricultural subsidies distributed since 2010 is `r misc_vars[misc_vars$var_name=="total_amount",]$val` HUF. From this amount `r misc_vars[misc_vars$var_name=="total_amount_address_sharing",]$val` went to beneficiaries where individuals and institutions share the address. This is more than `r floor((misc_vars[misc_vars$var_name=="total_amount_address_sharing",]$val)/(misc_vars[misc_vars$var_name=="total_amount",]$val)*100)`% of the total amount so it's worth checking if there is some pattern. The proportion of the amount of money received by firms versus individuals seems random for all settlement types except for Budapest. In the capital the share of individuals is lower for higher total amount.
...
ggplot(data = misc_same_addresses %>%
              filter(settlement_type %in% c("Budapest", "megyeszékhely")),
...
Here are the individuals who won at lease 1 billion HUF since 2010. Even though the names and addresses are available in the dataset I chose not to disclose those in this table.
...
When aggregating wins by addresses we get additional "agricultural subsidy billionares". Here are the top 10. It is interesting to note that this list is lead by an individual.
...
```{r fig.cap='Individuals who won more than 500 mm HUF', fig.subcap=c('Single winners', 'Individuals sharing the same address'), fig.ncol = 2, out.width = "50%", fig.align = "center"}
ggmap(hun_map) +
  geom_point(data = misc_ppl_wins_max, aes(x = longitude, y = latitude, size = total/1000000000), color = "purple", alpha = 0.5) +
  labs(size = "bn Huf", color = NULL)
...
ggmap(hun_map) +
  ...
  labs(size = "bn Huf", color = "winners") +
  theme_minimal() +
  scale_fill_viridis(discrete=TRUE, direction=1, option = "D", guide = guide_legend(reverse = TRUE))
