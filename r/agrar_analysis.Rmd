...
If we divide the data to 10 bins of equal size the proportion of the top decil is striking in this dataset too.

```{r fig.cap='Average and total of subsidies won by institutions by decils', fig.show = "hold", out.width = "50%", echo=FALSE, results='hide', fig.height=6}
...
The total amount of agricultural subsidies distributed since 2010 is `r f(misc_vars[misc_vars$var_name=="total_amount",]$val)` HUF. From this amount `r f(misc_vars[misc_vars$var_name=="total_amount_address_sharing",]$val)` went to beneficiaries where individuals and institutions share the address. This is more than `r floor((misc_vars[misc_vars$var_name=="total_amount_address_sharing",]$val)/(misc_vars[misc_vars$var_name=="total_amount",]$val)*100)`% of the total amount so it's worth checking if there is some pattern. The proportion of the amount of money received by firms versus individuals seems random for all settlement types except for Budapest. In the capital the share of individuals is lower for higher total amount.
...
Here are the individuals who won at lease 1 billion HUF since 2010. Even though the names and addresses are available in the dataset I chose not to disclose those in this table. It is interesting to note that the first and third persons are likely the same individuals.
...
Below are the top "agricultural subsidy billionares"; the list is created by aggregating wins by addresses. It is interesting to note that this list is lead by an individual.
...
We can see where they are located on the map of Hungary. For individual winners Budapest was barely visible on the map but for institutions the capital is clearly a center.
...
county_data_ <- misc_yearly_win_by_settlement %>%
  group_by(ksh_id) %>%
  ...
  mutate(amt_per_pop=amount/population,
         ...
         winner_per_pop=winners/population)

```

We can also do aggregate statistics by county. The below maps show the yearly averages of subsidies and recipients by

```{r echo = FALSE, message = FALSE, warning = FALSE, fig.cap=''}
ggmap(hun_map) +
  geom_point(data = county_data,
             aes(x = longitude, y = latitude, size = amount/1000000000, color = amt_per_area),
             alpha = 1) +
  labs(x = "lon", y = "lat", size = "bn Huf", color = "amt/area") +
  theme_minimal() +
  theme(legend.position = "right") +
...
ggmap(hun_map) +
  geom_point(data = county_data,
             aes(x = longitude, y = latitude, size = amount/1000000000, color = winner_per_pop*100),
             alpha = 1) +
  labs(x = "lon", y = "lat", size = "bn Huf", color = "winner %") +
  theme_minimal() +
  theme(legend.position = "right") +
  guides(color = guide_legend(override.aes = list(size = 8))) +
  scale_colour_gradient(low = "orange", high = "purple")
...
```{r echo = FALSE, message = FALSE, warning = FALSE, fig.cap=''}
county_data <- misc_yearly_win_by_settlement %>%
  select(-district,-homes,-settlement,-ksh_id,-settlement_type) %>%
