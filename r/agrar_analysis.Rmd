...
ggplot(data = sum_by_dummies, aes(x = amount/1000000000, y = year, fill = source)) + 
  geom_bar(stat = "identity") +
  xlab("sum of total amount (bn HUF)") +
  ylab("") +
  theme_minimal() +
...
ggplot(data = sum_by_dummies, aes(x = amount/1000000000, y = year, fill = source)) + 
  ...
  xlab("proportion of total amount (bn HUF)") +
  ylab("") +
  theme_minimal() +
...
- **Character cases**: Some names had multiple versions with inconsistent capitalizations (eg. all caps or 2 upper-case starting characters) so I split names by spaces and capitalized only the first character in each name part.
...
Individuals and institutions are not separated in the data. There is no indication of the beneficiary type but it would be important to investigate the 2 groups separately. I made the initial assumption that any name with 8 or more name parts is a firm but this is obviously not enough as it misses institutions with shorter names. For names that have 8 or fewer name parts I identified a large number of keywords that indicate that the beneficiary is an institution.

```{r, engine = 'bash', eval = TRUE}
paste -s -d" " ../input/names/firm_keywords.txt | fold -w 90
```

There are not just firms but churches, municipalities, clubs and all sorts of other organizations; for simplicity I call all of these firms. Their distribution is around 50% for all years.
...
ggplot(data = sum_by_dummies, aes(x = amount/1000000000, y = year, fill = is_firm)) + 
  geom_bar(stat = "identity") +
...
Most of the non-institutional beneficiaries are Hungarian citizens. It would be interesting to see the gender-based distribution of the subsidies given to people. This information is not in the data but we can split citizens to "male" and "female" categories based on their names. This will not be 100% accurate but the data does not enable a better approach. All given names that are legally allowed in Hungary are available on the website of the [Hungarian Scientific Academy](http://www.nytud.mta.hu/oszt/nyelvmuvelo/utonevek/). Names not in the list are of foreign origin so they will be categorized as 'unknown'. Some female citizens changed their legal names to that of their husbands by adding a "-né" postfix. This pattern is easy to find so that is also taken into account.
...
ggplot(data = by_gender, aes(x = amount/1000000000, y = year, fill = gender)) + 
  geom_bar(stat = "identity") +
  labs(x = "sum amount (bn HUF)", y="") +
  theme_minimal() +
  theme(legend.position = "none") +
...
The below table shows some descriptive statistics about the whole dataset. The range of subsidies is very large. The standard deviation is huge and the mean is more than 7 times as large as the median.
...
The distribution is very skewed so visualizing the nominal amounts is not meaningful. The below violin plot shows the payments for each year using a logarithmic scale of base 10. So a value of 4 means a payout of 10 000, while a value of 5 means 100 000. 
...
When we look at the yearly distribution of subsidies summed by beneficiaries we see a steady increase in the mean and median values. If we compare the number of data points per year with the previous section we see that the number of payments almost doubled since 2010 but the number of beneficiaries went down by more than 12000. This means that fewer beneficiaries receive more money but in smaller installments compared to earlier years.
...
```{r include=FALSE, echo = FALSE, message = FALSE, warning = FALSE}
rm(stat_controls, full_stat_table, full_stat_table, by_address_stat_table)
```

## Trends by beneficiary type
...
ggplot(data = sum_by_dummies, aes(x = amount/1000000000, y = year, fill = settlement_type)) + 
  geom_bar(stat = "identity") +
  labs(x = "sum amount (bn HUF)", y = "") +
  theme_minimal() +
  scale_fill_viridis(discrete=TRUE, direction=1, option = "D", guide = guide_legend(reverse = TRUE))
...
```{r fig.cap='Average amounts by decil', fig.subcap=c('Log of averages', 'Average amounts'), fig.ncol = 2, out.width = "50%", fig.align = "center", echo = FALSE, message = FALSE, warning = FALSE}
save_3d_chart(full_by_year, 10, "log_avg", "all_yearly")
save_3d_chart(full_by_year, 10, "avg", "all_yearly")
```

```{r fig.cap='Sum of amounts by decil', fig.subcap=c('Log of sum of wins', 'Sum of subsidies'), fig.ncol = 2, out.width = "50%", fig.align = "center", echo = FALSE, message = FALSE, warning = FALSE}
...
```{r fig.show = "hold", out.width = "50%", echo=FALSE, results='hide', fig.height=6, echo = FALSE, message = FALSE, warning = FALSE}
